(() => {
  const RUNTIME_VERSION = (() => {
    try { return chrome.runtime.getManifest().version; } catch (_) { return '0.6.9'; }
  })();
  ...
  function searchBox() {
    ...
    input.addEventListener('input', () => {
      ...
      state.quickAiProviderIds = new Set();
  ...
  function footer() {
    ...
    if (sources.length) {
      ...
      sources.forEach((source, index) => {
        const button = el('button', `dictfloat-quick-chip${source.enabled !== false ? ' active' : ''}`, `${source.enabled !== false ? '✅' : '☐'}${index + 1}`);
        button.type = 'button';
    ...
    const online = el('button', `dictfloat-quick-chip dictfloat-online-toggle${state.settings.onlineLookup ? ' active' : ''}`, `${state.settings.onlineLookup ? '✅' : '☐'}Online`);
    online.type = 'button';
    ...
    if (providers.length) {
      ...
      providers.forEach((provider, index) => {
        ...
        const active = state.quickAiProviderIds.has(id);
        const button = el('button', `dictfloat-quick-chip dictfloat-ai-chip${active ? ' active' : ''}`, `${active ? '✅' : '☐'}${index + 1}`);
        button.type = 'button';
        button.title = `${active ? 'Hide' : 'Run'} ${provider.providerName || `AI ${index + 1}`}`;
        button.addEventListener('click', () => toggleQuickAiProvider(id));
        row.append(button);
      });
    }
  ...
  function toggleQuickAiProvider(providerId) {
    const id = String(providerId || '');
    if (!id) return;
    const runtimeKey = `ai:${id}`;
    const query = state.query.trim();
    if (state.quickAiProviderIds.has(id)) {
      state.quickAiProviderIds.delete(id);
    ...
    const existing = state.translation.query === query
      ...
      : null;
    state.quickAiProviderIds.add(id);
    if (!query) {
      renderContentOnly();
      return;
    }
    ...
    runSentenceTranslation(query, [runtimeKey]).catch(handleAsyncError);
  }
  ...
  function shouldShowSentenceTranslationSection(raw) {
    ...
    if (state.quickAiProviderIds.size > 0) return true;
  ...
  function enabledTranslationProviders() {
    ...
    const availableIds = new Set(normalizeAiProviders(state.settings.aiProviders, state.settings).map((item) => String(item.id || '')));
    const ai = [...state.quickAiProviderIds]
  ...
  function appendSentenceTranslationSection(box) {
    ...
    const section = createSourceSection({
      ...
      badge: state.quickAiProviderIds.size ? 'Manual' : 'Auto',
      tone: 'translate'
    });
