(() => {
  ...
  function appendOrderedLookupSections(box) {
    ...
    let appended = 0;
    if (shouldOfferSentenceTranslation(query)) {
  ...
  function appendSentenceTranslationSection(box) {
    ...
    if (!providers.length) {
      ...
      if (active.status === 'idle' && !rows.length) {
        section.body.append(el('div', 'dictfloat-translate-hint', 'Press Enter or use the selection marker to translate this sentence automatically.'));
      }
  ...
  async function runSentenceTranslation(query, onlyProviders = null) {
    ...
    const providers = (Array.isArray(onlyProviders) && onlyProviders.length ? onlyProviders : enabledTranslationProviders()).filter(Boolean);
    if (!providers.length) {
      state.translation = { query: text, status: 'error', results: [], data: null, error: 'No translation provider is enabled.', provider: '' };
      renderContentOnly();
      return;
    }
    const results = providers.map((provider) => ({ provider, status: 'pending', data: null, error: '' }));
    state.translation = { query: text, status: 'loading', results, data: null, error: '', provider: providers[0] };
    renderContentOnly();
    ...
    for (const provider of providers) {
      ...
      const update = (patch) => {
        state.translation = {
          ...state.translation,
          results: state.translation.results.map((item) => item.provider === provider ? { ...item, ...patch } : item),
          provider
        };
        renderContentOnly();
      };
