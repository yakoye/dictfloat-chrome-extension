(() => {
  const RUNTIME_VERSION = (() => {
    try { return chrome.runtime.getManifest().version; } catch (_) { return '0.6.8'; }
  })();
  ...
  function footer() {
    ...
    const top = el('div', 'dictfloat-footer-row dictfloat-footer-main');
    ...
    top.append(settingsButton);

    const dictRow = quickDictionaryRow();
    const serviceRow = quickServiceRow();

    foot.append(top);
    if (dictRow) foot.append(dictRow);
    foot.append(serviceRow);
    return foot;
  }

  function quickDictionaryRow() {
    const sources = knownLookupSources().filter((source) => source.kind !== 'online');
    if (!sources.length) return null;
    const row = el('div', 'dictfloat-footer-row dictfloat-quick-row');
